...
from utils.consistency_aug import gen_mask, Filp_EMA, simple_augment_reverse
...
class Train(Dataset):
	...
	def __getitem__(self, index):
		...
		label_down1 = cv2.resize(lb, (0,0), fx=1/2, fy=1/2, interpolation=cv2.INTER_NEAREST)
		label_down1 = np.transpose(label_down1, (2,0,1))
		label_down2 = cv2.resize(lb, (0,0), fx=1/4, fy=1/4, interpolation=cv2.INTER_NEAREST)
		label_down2 = np.transpose(label_down2, (2,0,1))
		label_down3 = cv2.resize(lb, (0,0), fx=1/8, fy=1/8, interpolation=cv2.INTER_NEAREST)
		label_down3 = np.transpose(label_down3, (2,0,1))
		label_down4 = cv2.resize(lb, (0,0), fx=1/16, fy=1/16, interpolation=cv2.INTER_NEAREST)
		label_down4 = np.transpose(label_down4, (2,0,1))
		lb = np.transpose(lb, (2,0,1))
		lb_affs1 = seg_to_aff(label_down1).astype(np.float32)
		lb_affs2 = seg_to_aff(label_down2).astype(np.float32)
		lb_affs3 = seg_to_aff(label_down3).astype(np.float32)
		lb_affs4 = seg_to_aff(label_down4).astype(np.float32)
		...
		if self.if_ema_intensity:
			ema_imgs = self.intensity_aug(ema_imgs)
		if self.if_ema_mask:
		...
		down1 = np.concatenate([lb_affs1, weightmap1], axis=0)
		down2 = np.concatenate([lb_affs2, weightmap2], axis=0)
		down3 = np.concatenate([lb_affs3, weightmap3], axis=0)
		down4 = np.concatenate([lb_affs4, weightmap4], axis=0)
		down1 = np.ascontiguousarray(down1, dtype=np.float32)
		down2 = np.ascontiguousarray(down2, dtype=np.float32)
		down3 = np.ascontiguousarray(down3, dtype=np.float32)
		down4 = np.ascontiguousarray(down4, dtype=np.float32)

		return imgs, ema_imgs, lb_affs, weightmap, rule, down1, down2, down3, down4
	...
	def next(self):
		...
		try:
			...
			if self.is_cuda:
				...
				batch[6] = batch[6].cuda()
				batch[7] = batch[7].cuda()
				batch[8] = batch[8].cuda()
			return batch
...
if __name__ == '__main__':
	...
	np.random.seed(seed)
	random.seed(seed)
	cfg_file = 'seg_ct_embedding_mse_em1_sw.yaml'
	...
	for i in range(0, 50):
		t1 = time.time()
		tmp_data, ema_imgs, affs, weightmap, rules, down1, down2, down3, down4 = iter(data).__next__()
		print('single cost time: ', time.time()-t1)
		tmp_data = np.squeeze(tmp_data)
		ema_imgs = np.squeeze(ema_imgs)
		rules = np.squeeze(rules)
		eam_reverse = simple_augment_reverse(ema_imgs, rules)
		if cfg.MODEL.model_type == 'mala':
		...
		img_ema = show_one(ema_imgs)
		img_reverse = show_one(eam_reverse)
