...
from data.data_provider_labeled_deep import Provider
...
from model.unet3d_mala import UNet3D_MALA_deep as UNet3D_MALA
...
from loss.loss_embedding_mse import ema_embedding_loss_norm1, ema_embedding_loss_norm5
...
def loop(cfg, train_provider, valid_provider, model, criterion, optimizer, iters, writer):
    ...
    while iters <= cfg.TRAIN.total_iters:
        ...
        iters += 1
        t1 = time.time()
        inputs, ema_inputs, target, weightmap, rules, down1, down2, down3, down4 = train_provider.next()
        ...
        optimizer.zero_grad()
        emd1, emd2, emd3, emd4, embedding = model(inputs)
        _, _, _, _, ema_embedding = model(ema_inputs)
        ...
        if cfg.TRAIN.embedding_mode == 1:
            loss_embedding, pred = embedding_loss_norm1(embedding, target, weightmap, criterion, affs0_weight=cfg.TRAIN.affs0_weight)
            loss_embedding_cross, _ = ema_embedding_loss_norm1(embedding, ema_embedding, target, weightmap, criterion, affs0_weight=cfg.TRAIN.affs0_weight)
        elif cfg.TRAIN.embedding_mode == 5:
            loss_embedding, pred = embedding_loss_norm5(embedding, target, weightmap, criterion, affs0_weight=cfg.TRAIN.affs0_weight)
            loss_embedding_cross, _ = ema_embedding_loss_norm5(embedding, ema_embedding, target, weightmap, criterion, affs0_weight=cfg.TRAIN.affs0_weight)
        else:
            raise NotImplementedError
        loss_emd1, _, = embedding_loss_norm1(emd1, down4[:,:3], down4[:,3:], criterion, affs0_weight=cfg.TRAIN.affs0_weight)
        loss_emd2, _, = embedding_loss_norm1(emd2, down3[:,:3], down3[:,3:], criterion, affs0_weight=cfg.TRAIN.affs0_weight)
        loss_emd3, _, = embedding_loss_norm1(emd3, down2[:,:3], down2[:,3:], criterion, affs0_weight=cfg.TRAIN.affs0_weight)
        loss_emd4, _, = embedding_loss_norm1(emd4, down1[:,:3], down1[:,3:], criterion, affs0_weight=cfg.TRAIN.affs0_weight)
        loss = loss_embedding + loss_embedding_cross + loss_emd1 + loss_emd2 + loss_emd3 + loss_emd4
        loss.backward()
        shift = 1
        ...
        if iters % cfg.TRAIN.display_freq == 0 or iters == 1:
            ...
            if iters == 1:
                ...
                logging.info('step %d, emd=%.6f, emb_cross=%.6f' % (iters, loss_embedding, loss_embedding_cross))
