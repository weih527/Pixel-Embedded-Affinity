...
def loop(cfg, train_provider, valid_provider, model, ema_model, optimizer, iters, writer):
    ...
    while iters <= cfg.TRAIN.total_iters:
        ...
        # cfg.TRAIN.mask_weight * criterion_mask(pred_mask, torch.gt(target_ins[:, 0], 0), weight_rate=[10, 1]).to(device)
        ...
        if iters % cfg.TRAIN.display_freq == 0 or iters == 1:
            ...
            if iters == 1:
                ...
                logging.info('step %d, emd=%.6f, emb_cross=%.6f' % (iters, loss_embedding_total, loss_embedding_cross_total))
