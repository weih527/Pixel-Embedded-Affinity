...
def poolfeat(input, prob, sp_h=2, sp_w=2):
    ...
    #prob.narrow(1, 0, 1) 取第1维的0和1的两个channel
...
def update_spixl_map (spixl_map_idx_in, assig_map_in):
    ...
    b,_,h,w = assig_map.shape #(1,9,512,512)
    _, _, id_h, id_w = spixl_map_idx_in.shape #(1,9,512,512)
    ...
    assig_max,_ = torch.max(assig_map, dim=1, keepdim= True) #每个像素相邻的九个pixel中最相似的一个 (1,1,512,512)
