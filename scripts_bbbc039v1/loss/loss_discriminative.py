...
def discriminative_loss(embedding, seg_gt, delta_v=0.5, delta_d=1.5, alpha=1, beta=1, gama=0.001):
    ...
    for b in range(batch_size):
        ...
        if num_id > 1:
            ...
            dist = dist + torch.eye(num_id, dtype=dist.dtype, device=dist.device) * (2*delta_d)  # diagonal elements are 0, now mask above delta_d
